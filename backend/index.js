...
app.use(cors({
    origin: function (origin, callback) {
        ...
        if (allowedOrigins.includes(origin) || (process.env.VERCEL && origin.endsWith('.vercel.app'))) {
            return callback(null, true);
        }
        return callback(null, true); // Fallback to allow (more permissive for debugging)
    },
    credentials: true
}));
