...
// Lazy load geoip-lite only when needed to save memory
let geoip;
...
const getLocationFromIP = (req) => {
    try {
        // 1. Check Vercel headers first (lightweight & accurate)
        ...
        // 2. Fallback to geoip-lite (heavy)
        ...
        console.error('GeoIP Fallback Error:', error.message);
        return { country: 'Unknown', city: 'Unknown', region: 'Unknown' };
    }
};
...
exports.initSession = async (req, res) => {
    try {
        ...
        */
        ...
        // Get location from IP using GeoIP or Vercel Headers
        visitor.location = getLocationFromIP(req);
        await visitor.save();
